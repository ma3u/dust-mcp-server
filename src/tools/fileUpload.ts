...
async function ensureUploadsDir() {
  try {
    ...
    throw new Error(
      `Failed to create uploads directory at ${UPLOADS_DIR}: ${errorMessage}`
    );
  }
}
...
export default (server: McpServer) => {
  ...
  server.tool(
    ...
    'Upload a document (PDF or image) for processing',
    {
      ...
      fileName: z.string({
        description:
          "Original file name with extension (e.g., 'lab_results.pdf')",
      }),
      fileType: z.enum(
        ['report', 'correspondence', 'data_analysis', 'general'],
        {
  ...
  server.tool(
    ...
    {
      fileType: z
        .enum(['report', 'correspondence', 'data_analysis', 'general', 'all'], {
      ...
      limit: z
        ...
        .optional(),
      sortBy: z
        .enum(['uploadDate', 'fileName', 'fileSize'], {
      ...
      try {
        ...
        if (params.fileType && params.fileType !== 'all') {
          filteredDocuments = documents.filter(
            (doc) => doc.fileType === params.fileType
          );
        }
