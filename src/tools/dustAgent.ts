...
const dustAgentTool = (server: McpServer) => {
  ...
  const queryDustAgentSchema = {
    agentId: z
      .string({
        description:
          'ID of the Dust agent to query (optional, will use the first agent from DUST_AGENT_IDs if not provided)',
      })
      .optional(),
    query: z.string({
    ...
    documentIds: z
      .array(z.string(), {
        description:
          'Optional IDs of processed documents to include as context',
      })
      .optional(),
    context: z
      .record(z.any(), {
    ...
    echo: z
      .boolean({
        description:
          'If true, echo back the query instead of getting a real response from the agent (for testing)',
      })
      .optional(),
  };
  ...
  server.tool(
    ...
    async (params: DustAgentParams, _extra) => {
      ...
      try {
        ...
        // If document IDs are provided, add them to the context
        if (params.documentIds && params.documentIds.length > 0) {
          // In a real implementation, you would fetch document data here
          // For now, we'll just pass the document IDs with minimal metadata
          context.documents = params.documentIds.map((docId) => ({
            id: docId,
            name: `Document ${docId}`,
            fileType: 'unknown',
            extractedText: '',
        ...
        if (!agentId) {
          throw new Error(
            'No agent ID provided and no default agent ID found in DUST_AGENT_IDs environment variable'
          );
        }
        ...
        const mcpResponse: MCPResponse = createTextResponse(responseText, {
          agentId: agentId,
          query: params.query,
  ...
  const queryMultipleAgentsSchema = z.object({
    ...
    conversationId: z
      .string({
        description:
          'Optional conversation ID for continuing a previous conversation',
      })
      .optional(),
    echo: z
      .boolean({
        description:
          'If true, echo back the query instead of getting real responses',
      })
      .optional()
      .default(false),
  });
  ...
  server.tool(
    ...
    async (params) => {
      type QueryMultipleAgentsParams = z.infer<
        typeof queryMultipleAgentsSchema
      >;
      ...
      if (params.documentIds?.length) {
        ...
        Object.assign(context, {
          documents: params.documentIds.map((docId) => ({
            id: docId,
            name: `Document ${docId}`,
            fileType: 'unknown',
            extractedText: '',
          ...
          try {
            ...
            if (params.echo) {
              return {
                agentId,
                content: [
                  { type: 'text' as const, text: `ECHO: ${params.query}` },
                ],
                _meta: {
                  echo: true,
                  query: params.query,
            ...
            return {
              ...
              content: [
                {
                  type: 'text' as const,
                  text: `Error querying agent ${agentId}: ${errorMessage}`,
                },
              ],
              isError: true,
              _meta: {
      ...
      try {
        ...
        if (!agents || agents.length === 0) {
          return createTextResponse(
            'No Dust agents found. Please check your API key and configuration.'
          );
        }
        ...
        // Format the response
        const agentsList = filteredAgents
          .map(
            (agent) =>
  ...
  server.tool(
    ...
    async (params: GetAgentInfoParams) => {
      try {
        ...
        if (!agent) {
          return createErrorResponse(
            `Agent with ID ${params.agentId} not found`,
