...
export class DustApiService {
  ...
  async listAgents(): Promise<DustAgent[]> {
    try {
      const response = await this.client.get(
        `/workspaces/${this.workspaceId}/agents`
      );
  ...
  async getAgent(agentId: string): Promise<DustAgent> {
    try {
      const response = await this.client.get(
        `/workspaces/${this.workspaceId}/agents/${agentId}`
      );
  ...
  async endSession(sessionId: string): Promise<void> {
    try {
      await this.client.delete(
        `/workspaces/${this.workspaceId}/sessions/${sessionId}`
      );
    } catch (error) {
