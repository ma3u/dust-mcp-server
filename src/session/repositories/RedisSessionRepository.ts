...
export class RedisSessionRepository implements SessionRepository {
  ...
  async update(sessionId: string, { data, ttl = this.defaultTTL }: UpdateSessionInput): Promise<ISession | null> {
    const existing = await this.findById(sessionId);
    if (!existing) return null;

    const now = new Date();
    const expiresAt = ttl ? new Date(now.getTime() + ttl * 1000) : existing.expiresAt;
