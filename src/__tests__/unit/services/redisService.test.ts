...
describe('RedisConnection', () => {
  ...
  describe('connection events', () => {
    it('should log on successful connection', () => {
      ...
      const connectHandler = (
        MockedIORedis.mock.instances[0] as any
      ).on.mock.calls.find(([event]: [string]) => event === 'connect')[1];
    ...
    it('should handle connection errors', () => {
      ...
      const errorHandler = (
        MockedIORedis.mock.instances[0] as any
      ).on.mock.calls.find(([event]: [string]) => event === 'error')[1];

      // Act
      errorHandler(testError);

      // Assert
      expect(errorSpy).toHaveBeenCalledWith('Redis connection error', {
  ...
  describe('connection management', () => {
    ...
    it('should handle errors during connection close', async () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith('Error closing Redis connection', {
        error,
      });
    });
  });
