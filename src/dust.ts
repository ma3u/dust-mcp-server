...
function logger(level: string, message: string, data?: any): void {
  ...
  // Write to log file
  fs.appendFileSync(
    path.join(LOG_DIR, `mcp-${new Date().toISOString().split('T')[0]}.log`),
    logMessage + '\n'
  );
    ...
    'dust_agent_query',
    {
      ...
      parameters: z.object({
        agent_id: z
          ...
          .optional()
          .describe(
            'ID of the agent to query (optional, will use default if not provided)'
          ),
        ...
        context: z
          .record(z.any())
          .optional()
          .describe('Optional context for the agent'),
      }),
    },
    async ({ agent_id, query, conversation_id, context }) => {
      logger('INFO', 'Received dust_agent_query request', {
        agent_id,
    ...
    'dust_list_agents',
    {
      ...
      parameters: z.object({
        view: z
          .string()
          .optional()
          .describe('Optional view type for filtering agents'),
        limit: z
          .number()
          .optional()
  ...
  server.tool(
    'dust_get_agent_config',
    {
      ...
      parameters: z.object({
        agent_id: z
          .string()
          .describe('ID of the agent to get configuration for'),
      }),
    },
    async ({ agent_id }) => {
      ...
      try {
        ...
        logger(
          'INFO',
          `Successfully retrieved configuration for agent ${agent_id}`
        );
        ...
        if (
          ...
          config.supportedOutputFormats.length > 0
        ) {
