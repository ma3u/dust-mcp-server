...
function logger(level, message, data) {
  ...
  fs.appendFileSync(
    path.join(
      LOG_DIR,
      `test-dust-${new Date().toISOString().split('T')[0]}.log`
    ),
    logMessage + '\n'
  );
...
async function testDustAPI() {
  try {
    ...
    if (!DUST_API_KEY || !DUST_WORKSPACE_ID) {
      logger(
        'ERROR',
        'Missing required environment variables: DUST_API_KEY, DUST_WORKSPACE_ID'
      );
      return;
    }
          ...
          if (messageId) {
            ...
            while (!isComplete && attempts < maxAttempts) {
              ...
              try {
                ...
                logger('INFO', `Message status (attempt ${attempts}):`, {
                  status,
                });
            ...
            if (!isComplete) {
              logger(
                'ERROR',
                'Message did not complete within the expected time'
              );
            }
          }
        } catch (error) {
